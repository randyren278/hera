...
# at least one settings.json.brain-backup.* is free of second-brain hooks
...
# Seed a pristine settings.json with an unrelated hook (no second-brain refs).
...
clean_found=0
for b in "$SCRATCH"/.claude/settings.json.brain-backup.*; do
  [ -e "$b" ] || continue
  if ! grep -q "second-brain.env" "$b"; then
    clean_found=1
...
if [ "$clean_found" -ne 1 ]; then
  ...
  ls -la "$SCRATCH"/.claude/settings.json.brain-backup.* 2>/dev/null
  exit 1
fi
