...
#
# Pins today's state before the fix: /brain-team retrieve DOES pull before
...
SKILL="$REPO/.claude/skills/brain-team/SKILL.md"
SYNC="$REPO/scripts/team_sync.py"

# 1. retrieve already has a pre-search sync step.
if ! grep -q 'clone-or-pull' "$SKILL"; then
  echo "FAIL: brain-team SKILL.md has no clone-or-pull sync step"; exit 1
fi
