---
name: hera-team
description: All team space operations in one place. add (publish your redacted pages up to the shared repo), remove (un-publish your own pages — real delete, owner-scoped, human-gated), retrieve (search the team space + your vault), pull (sync). Use when the user says "share/publish this to team", "remove/unpublish my team pages", "what does the team know about X", "what did <teammate> publish", or "search the team space".
---

# hera-team

One skill for the whole team space loop. Your redacted knowledge goes **up**
...
|---|---|---|
| "share/publish this to team", "add to team space" | `add <path>` | `publish.py` |
| "remove / unpublish my team pages", "pull my stuff back" | `remove` | `team_remove.py` + `publish.py push` |
| "what does the team know about X", "search team space" | `retrieve [--owner X] <q>` | `team_sync.py` + `team_search.py` |
| "what did <teammate> publish about Y" | `retrieve --owner <name> <q>` | same, owner-scoped |
| "sync the team space" | `pull` | `team_sync.py` |

---

## add `<path>` — publish your redacted knowledge up

Publish your redacted knowledge up to the team space. Manual-only; nothing
reaches the shared repo without the human diff review.

1. Private ingest: `python "<VAULT>/scripts/hera_cli.py" ingest <path>`
   (or delegate to `/hera-ingest`). Writes the private pages, enqueues conflicts.
2. Strip + stage: `python "<VAULT>/scripts/hera_cli.py" publish stage <path> --json`.
...
`team-staging/<you>/`), and **human-gated** (same as add). Git history
is the undo — there is no `.archive` mirror on the team side.

1. Fresh view: `python "<VAULT>/scripts/hera_cli.py" team_sync clone-or-pull`.
...
2. List *your* published pages: `python "<VAULT>/scripts/hera_cli.py" team_remove list --json`.
...
4. Stage the deletions: `python "<VAULT>/scripts/hera_cli.py" team_remove stage-remove <path>...`.
...
## retrieve `[--owner <name>] <query>` — search the team space + your vault

1. Sync first (pull only): `python "<VAULT>/scripts/hera_cli.py" team_sync clone-or-pull`.
...
2. Search: `python "<VAULT>/scripts/hera_cli.py" team_search [--owner <name>] --json "<query>"`.
...
- **`remove` is owner-scoped** — only ever `team-staging/<you>/`. You cannot
...
- **No hera.db writes for team pages** — team content stays out of your personal
  ...
  but never writes team pages, vectors, or citations into `hera.db`.
