---
name: brain-setup
description: One-shot installer for a fresh clone of the second-brain repo — scaffolds wiki/, creates brain.db from the §5 schema, verifies Ollama + nomic-embed-text, wires the hooks (§8), and (CP-7 onward) clones the team-brain staging repo. Also exposes `--doctor` for health checks.
---

# brain-setup

Clone-and-go installer for the Second Brain. Run once after cloning the repo.
...
locator `~/.claude/second-brain.env`). This skill is the installer, so on a
...
1. **Environment preflight.** Run `python "<VAULT>/scripts/brain_cli.py" preflight`.
...
3. **Database.** `python "<VAULT>/scripts/brain_cli.py" brain_db --init`
   creates `brain.db` with the §5 schema and default config keys.
...
6. **team-brain staging (optional).** Ask the user: **"Set up a team space? (y/N)"**
   A team space lets you publish redacted pages to — and search — a shared
   team-brain git repo. It is entirely optional; the single-user brain works
   fully without one.

   - **If no:** state that team features stay dormant. `/brain-team` commands
     will cleanly no-op with `No team space configured — run /brain-setup to
     add one.` until a remote is configured. Skip the rest of this step.

   - **If yes:** prompt for the team-brain git repo URL, then verify access
     *before* wiring anything (read-only — never clones on a bad URL):

     ```
     python "<VAULT>/scripts/brain_cli.py" team_sync check "<url>"
     ```
     ...
     appending (or replacing) this line in `~/.claude/second-brain.env`,
     beside the existing `SECOND_BRAIN_VAULT` (plain `KEY=VALUE`, no `export`):

     ```
     SECOND_BRAIN_TEAM_REMOTE="<url>"
     ```

     Then bring the shared brain to disk and ensure the owner's folder exists.
     `team_sync.py` reads the remote from `SECOND_BRAIN_TEAM_REMOTE`; it
     ...
     `<VAULT>/team-brain-staging/<owner>/` (owner = `BRAIN_OWNER`, default
     `randy`).

     The owner folder is named from `BRAIN_OWNER` (default `randy`) — the
   ...
   `/brain-team add` stages redacted pages under
   `team-brain-staging/<owner>/` and requires human diff review + explicit
   approval before any push (ADR-08). `/brain-team retrieve` uses the same
   ...
   team brain. Re-run `/brain-setup` any time to add or change it.

7. **Global CLAUDE.md (optional).** `install.py` step 7 offers to append the
   vault's `CLAUDE.md` into `~/.claude/CLAUDE.md` so the brain's *behavioral*
   rules (cite what you use as `(Source: [[Title]])`, never auto-push to
   team-brain, use the engines) stay active in **every** directory — not just
...
`python "<VAULT>/scripts/brain_cli.py" brain_db --doctor`
...
for?" and write the answer into `$SECOND_BRAIN_VAULT/wiki/overview.md`. This
