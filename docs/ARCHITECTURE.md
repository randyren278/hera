# ARCHITECTURE: how Second Brain fits together
...
- **`brain.db`**: a SQLite index (FTS5 for BM25 + sqlite-vec for dense embeddings + bookkeeping tables). Derived from `wiki/`; rebuildable. Never the source of truth for *content*, but the source of truth for *rankings, citations, conflicts, and cursors*.
- **`scripts/` engines**: the Python that does the heavy lifting: ingest, search, conflict resolution, prune, publish, and the team-brain sync/search/remove engines. Invoked by hooks and by the `brain-*` skills.
...
The one surprising property to hold in your head: **hooks act on their own.** Two of them write to `brain.db` as a side effect of normal use (scoring your citations, filing your session). Every hook is **fail-open**: if it errors, it prints nothing and returns 0, never blocking your session. That fail-open posture is an invariant, not a nicety (see [§5](#5-hard-invariants)).

---

## 2. Component topology

The dependency map. `wiki/` is the content store; `brain.db` is the derived index; the `scripts/` engines mediate every write to `wiki/` (through `locks.py`) and own the index; the four hooks are Claude Code's entry points into the engines. The optional **team layer** is shown too: its stores (`team-brain-staging/`, `team.db`) are local files, and the single off-machine box is the team git remote.
...
flowchart TD
  ...
  subgraph localstores["on your machine"]
    ...
    DB[("brain.db &#40;index of YOUR pages&#41;")]
    STAGE[("team-brain-staging/ &#40;pulled Markdown&#41;")]
...
- **`publish.py` is deliberately not wired to any hook.** It only runs when you invoke `/brain-team add`, and its push step is a separate CLI subcommand gated on human approval.
- **The team layer is optional and lives entirely on your machine except the remote.** `team.db` and `team-brain-staging/` are local files (git-ignored / a local clone); only the **team git remote** is off-machine, and it is crossed solely by a read-only `pull` or an approval-gated `push`. `team.db` and `brain.db` are distinct files that never merge (the isolation invariant, [ADR-14](DECISIONS.md); see also [RETRIEVAL.md § what lives where](RETRIEVAL.md#3-what-lives-where)).
...
sequenceDiagram
  ...
  U->>CC: end session
  Note over H,E: session_end_file.py &#40;async, 600s&#41;: forks worker,<br>distills transcript to .brain/session-ID.md,<br>ingests it as source_kind=session
```
...
|---|---|---|---|
| Never auto-push `team-brain-staging/` | Leaks private notes to the team remote | `publish.py`: `push` is a separate CLI subcommand; `stage`/`diff` never call it | CATASTROPHIC |
| Never edit `brain.db` by hand outside `scripts/` | A raw connection won't load sqlite-vec; vector queries break | Open only via `brain_db.connect()` | CATASTROPHIC |
...
| Never bypass `$SECOND_BRAIN_VAULT` | Hard-coding a path breaks global mode and points a machine at the wrong vault | Hooks/skills read the env var, written by `install.sh` to `~/.claude/second-brain.env` | CATASTROPHIC |
| Never let team content into `brain.db` (ADR-14) | Team pages in `brain.db` would earn citations, count toward prune, and collide with your conflicts — polluting your local ranking | `team_index.py` opens only `TEAM_DB`; the two DBs fuse at query time but never merge | DEGRADES QUALITY |
...
| Injection produces no pointers | Ollama down / empty vault / prompt matched the coding heuristic | Doctor's Ollama check; `.brain/` has no inject log (inject is silent) | Start Ollama, pull `nomic-embed-text`; injection fail-opens by design |
| Citations not scoring | Stop hook errored, or no final-answer wikilinks | `.brain/scorer.log` | Re-run the turn; check `(Source: [[Title]])` syntax; titles must resolve in `pages` |
| Session not filed | SessionEnd worker errored, or already filed | `.brain/filing.log`; `filed_sessions` row | Idempotent, so safe to re-invoke; delete the `filed_sessions` row only to force a re-file |
| Hooks not firing at all | `$SECOND_BRAIN_VAULT` unset / bad symlinks | `~/.claude/second-brain.env`; `readlink ~/.claude/hooks/*.py` | Re-run `install.sh`; env file must `export SECOND_BRAIN_VAULT` |
| Stale conflict warnings at session start | Open conflicts scoped to this project's `cwd` | SessionStart output; `/brain-conflicts list` | Resolve via `/brain-conflicts` |
...
- `.brain/scorer.log` : Stop-hook citation scoring (`$BRAIN_SCORER_LOG`)
- `.brain/filing.log` : SessionEnd filing (`$BRAIN_FILING_LOG`)
