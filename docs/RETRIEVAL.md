...
Both retrievers read the same pages, indexed two ways in `brain.db`. `search.hybrid_search(conn, query)` runs them and fuses the results.
...
flowchart TD
  subgraph local["LOCAL — everything here is a file on your machine"]
    ...
    subgraph mine["your notes"]
      ...
      BDB[("brain.db<br><i>index of YOUR pages</i>")]
    end

    subgraph theirs["teammates' notes (a working git clone)"]
      STAGE["team-brain-staging/&lt;owner&gt;/<br><i>redacted Markdown, pulled down</i>"]
...
| `brain.db` | SQLite index | local (git-ignored) | yours | no (rebuildable from `wiki/`) | BM25 + vector index of *your* pages, plus citations, conflicts, and cursors (saved per-session progress markers for the scorer, not SQLite cursors). Source of truth for *rankings*, not content. |
| `team-brain-staging/<owner>/` | Markdown | local (a git clone) | everyone's | **yes** (team content) | Redacted pages pulled down from the remote, one folder per publisher. |
...
| team git **remote** | git repo | **remote** | shared | **yes** (shared truth) | The one off-machine store. `team-brain-staging/` clones from it (read) and pushes to it (only on your explicit approval). |

Two boundaries that a refactor must never cross:

- **`brain.db` and `team.db` are distinct local files.** `team_index.py` opens only `TEAM_DB` (`$BRAIN_TEAM_DB` or `<repo>/team.db`); the personal engines open `brain.db`. No team page, ULID row, or citation ever enters `brain.db`. That separation is what lets team retrieval surface *everyone's* pages without polluting your local ranking. See [§5](#5-the-isolation-invariant).
...
Both `brain.db` and `team.db` are runtime state (git-ignored, rebuildable) that live on your disk. The Markdown — `wiki/` and `team-brain-staging/` — is the durable truth; the remote is the shared copy of the latter.
...
sequenceDiagram
  ...
  participant BDB as brain.db
  participant PI as prompt_inject

  Note over R,IX: PULL SIDE — on /brain-team retrieve or pull, never per query
  SY->>R: git clone or pull --ff-only
  R-->>SY: redacted Markdown under team-brain-staging
  ...
  Note over BDB,PI: QUERY SIDE — every prompt, and /brain-team retrieve
...
**Pull side.** `team_sync.clone_or_pull()` resolves the remote from `SECOND_BRAIN_TEAM_REMOTE` (per-machine; no fixed remote — unset means every team path is a clean no-op), clones or fast-forward-pulls into `team-brain-staging/`, then calls `_reindex_after_sync()`. `team_index.reindex(changed_only=True)` walks `team-brain-staging/<owner>/`, upserts `pages`/`pages_fts`/`pages_vec` plus a `page_meta(page_id, owner, source, rel_path, mtime)` row for each page carrying a ULID `id:`, re-embeds only pages whose `mtime` moved, and drops rows whose file disappeared. `team_index` is imported lazily and a reindex failure only warns — it never fails the sync.
...
- **`team_search.py`** (`/brain-team retrieve`): the same fusion, but `--owner NAME` restricts to one teammate and excludes your personal vault (you asked for *their* view). Personal hits are labeled owner `me`, source `personal`.
...
`team_hybrid_search` differs from local `hybrid_search` in exactly one way: it joins `page_meta` to attach `owner`/`source` and returns dicts (a team hit carries an `owner`; a local `Hit` does not). The retrieval substrate — BM25 + dense + `_rrf_fuse` + floor — is the same code. See [PIPELINES.md § team-brain retrieval](PIPELINES.md#6-team-brain-retrieval-scriptsteam_indexpy-scriptsteam_searchpy) and [DATA-MODEL.md § team.db](DATA-MODEL.md#8-teamdb-the-team-brain-index).

---

## 5. The isolation invariant

**Team content is indexed only in `team.db`; it never enters your personal `brain.db`.** This is ADR-14 and one of the hard "never do this" rules.

- `team_index.py` opens `brain_db.connect(TEAM_DB)` — reusing the connection helper (so `sqlite-vec` and WAL load) but pointed at a *different file*. It never opens `brain.db`.
- `page_meta` (owner/source/rel_path/mtime) is created only in `team.db`, never in `brain_db.SCHEMA`, so team-only columns stay out of the personal DB.
- Retrieval fuses the two DBs at *query time* by score. Fusion reads both; it merges neither store into the other.

Why it matters: if team pages leaked into `brain.db`, they would earn citations, count toward prune, and collide with your own conflicts — mixing other people's notes into your local ranking. Keeping the stores separate is what lets team retrieval surface everyone's pages while your personal ranking stays entirely yours. See [DECISIONS.md ADR-14](DECISIONS.md#2-adr-log-01-14) and [CLAUDE.md § What NEVER to do #9](../CLAUDE.md).
...
- The team sync/index/search engines in depth: [PIPELINES.md § team-brain retrieval](PIPELINES.md#6-team-brain-retrieval-scriptsteam_indexpy-scriptsteam_searchpy)
- The stores and the `team.db` schema: [DATA-MODEL.md § team.db](DATA-MODEL.md#8-teamdb-the-team-brain-index)
