...
- **It's local-first and single-user.** Nothing leaves your machine unless you explicitly say "push" to the team space.
...
- Ingest a source (`/hera-ingest`)
- Prune the vault (`/hera-prune`)
- Resolve conflicts (`/hera-conflicts`)
- Team space ops (`/hera-team add|remove|retrieve`) — publish your redacted
  pages up, un-publish your own (owner-scoped, human-gated delete), or search
  the team space by owner alongside your vault.
...
|---|---|---|---|---|---|
| `session_start.py` | Session starts | Shows you recent context from `hot.md`; warns about open conflicts from this project and stale pending deltas | Reads `hera.db`, `wiki/hot.md` | Fail-open: if the DB isn't there yet, prints nothing | The context block at the top of a new session |
| `prompt_inject.py` | You submit a prompt | Surfaces relevant vault pages as pointer lines so you don't have to remember them | Reads `hera.db` | Fail-open (10 s timeout): any error, whether Ollama is down or the vault is empty, injects nothing and you never notice | The "Relevant vault pages" block under your prompt |
| `stop_score.py` | You finish a turn | Records which pages you cited so useful ones surface first | Writes `citations` in `hera.db` | Async, never blocks; errors go to the log and it still returns cleanly | `.hera/scorer.log`; new rows in `citations` |
| `session_end_file.py` | Session ends | Distills the transcript into a page and ingests it, so past sessions become searchable | Writes `.hera/session-<id>.md`, ingests it | Async, fire-and-forget, **idempotent** (safe to retry via `filed_sessions`) | `.hera/filing.log`; a new `.hera/session-<id>.md` |

**Where to watch:** the two async hooks log to `.hera/scorer.log` (scoring) and `.hera/filing.log` (filing). The two sync hooks don't log; you see their output directly in the conversation. If a filed session looks wrong, it's reversible: re-filing is idempotent, and prune moves pages to `wiki/.archive/` rather than deleting them.
...
The setup skill bootstraps the Python `.venv`, creates `hera.db`, scaffolds `wiki/`, verifies Ollama, and wires the hooks. In Claude Code, from the vault, type:

```
/hera-setup
```

✓ Success: setup reports the venv built, `hera.db` created, and Ollama reachable.
...
✗ If instead you see `FAIL` on schema/tables, re-run `/hera-setup`.
...
| Hooks never fire | Hooks not registered — for a global install they live in `~/.claude/settings.json`; for a project-local install, in the vault's `.claude/settings.json` | `--doctor` check 9 verifies hook registration in both modes (global first); if it warns "not registered for THIS vault", run `install.py` from the vault or re-run `/hera-setup` |
...
- **vault**: the Markdown notes directory (`wiki/`) plus its `hera.db` index. We call it the *vault* throughout.
...
- **hybrid search**: the retrieval step recall runs — BM25 keyword search and dense-vector search over the same pages, combined with RRF. `search.hybrid_search` runs it over your `hera.db`; `search.team_hybrid_search` runs the same substrate over `team.db`.
...
- **team.db**: a second SQLite index, **local and git-ignored just like `hera.db`**, that indexes teammates' published pages pulled into `team-staging/`. It never merges with `hera.db` (the isolation invariant); the two are only fused at query time. The one genuinely off-machine store is the team git *remote*, not `team.db`.
...
- **conflict / freeze-on-ingest**: when an ingest would contradict an existing page, the new version is **not** written; the page on disk stays untouched and the contradiction is queued as an open conflict for you to resolve with `/hera-conflicts`.
